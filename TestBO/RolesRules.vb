...
''' <summary>
''' Summary description for RolesRules
''' </summary>
Partial Public Class RolesRules
    ...
    Public Overrides Function CheckRulesHook(ByVal ds As DataSet, ByVal tableName As String) As Boolean
        If ds.Tables(tableName).Rows.Count > 0 Then
			...
			If Row.RowState <> DataRowState.Unchanged AND Row.RowState <> DataRowState.Deleted Then
				Dim Entity As New RolesEntity()
				Entity.SetDataRow(ds.Tables(tableName).Rows(Me.CurrentRow))

				'' Call validation methods
				Me.ValidateDescription(Entity.Description)
			End If
	...
	''' </summary>
	Public Function ValidateDescription(description As String) As String
		Dim Msg As String = Nothing
		If mmType.IsEmpty(description) Then
			Me.EntityPropertyDisplayName = "Description"

			Msg = Me.RequiredFieldMessagePrefix + _
				Me.EntityPropertyDisplayName + _
				Me.RequiredFieldMessageSuffix

			AddErrorProviderBrokenRule("Description",  Msg)
		End If
		Return Msg
	End Function
